...
async def add_note(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Store Note, show summary, and save to CSV"""
    if update.message.text == 'Skip':
        # Only set N/A if not already set
...
async def update_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Create keyboard with column names (excluding Serial ID)
    columns = [col for col in CSV_COLUMNS if col != 'Serial ID']
...
async def update_field(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if field not in CSV_COLUMNS or field == 'Serial ID':
...
def main():
    ...
    token = '8533372557:AAH_BDUNX6m5lrAcLX7IaqsaZEKbcIZYmrE'
